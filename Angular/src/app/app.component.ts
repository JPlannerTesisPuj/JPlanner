...
export class AppComponent implements OnInit {
  ...
  doneListilla: any = null;
  
  //Array con los tokens de usuarios: primeros 6 son verdaderos, 7 al 12 no lo son
  private tokenArray = ["eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJKdWFuIEZlbGlwZSBIZXJuYW5kZXoiOiJibGEifQ.gpoUucL8_m7DSG_9IOjKAtfRfi3tX1ISyq5Ex46Muwg",
  ...
  ngOnInit() {
    ...
    this.readJSONFileService.filterToken(this.userToken).subscribe(user => {
      if(user.length == 0){
        ...
        this.user_authenticaded = user;
        this.name = "Bienvenido, " + this.user_authenticaded[0].nombre_estudiante;
      }
      });
  }
  ...
  generateToken(){
    var token_index = Math.floor((Math.random() * 11) + 1);
    this.userToken = this.tokenArray[token_index];
  }
}
